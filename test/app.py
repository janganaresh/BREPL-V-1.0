    ...
    try:
        for table in table_id_list:
            for pile_no in range(1, 5):  # Create 4 rows per Table ID with Pile No 1-4
                ...
                query = """
                INSERT INTO `assessment` (`Assessment ID`, `Area ID`, `User ID`, `Table ID`, `Pile No`, `Task Date`, 
                                          `Allotted Date`, `Allotted By`, `Date Completed`, `Assessment Status`)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """
                cursor.execute(query, (new_assessment_id, area_id, user_id, table, pile_no, task_date, allotted_date, allotted_by, date_completed, "In Progress"))
...
def update_assessment_pics():
    try:
        ...
        for i in range(4):  # Loop through 4 images
            ...
            if image:
                image_filename = f"{assessment_id}_{table_id}_Pile_{i+1}.jpg"  # Use Pile ID
...
def generate_report():
    ...
    try:
        ...
        if area_id and area_id.lower() != "all":
            query += " AND a.`Area ID` = %s"  # ✅ Filter by Area ID
